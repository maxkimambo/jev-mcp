...
/**
 * Resolve a directory to search. It must be an allowed root or sit below one,
 * both as named and after symlinks are resolved. Returns the real path.
 */
export async function resolveSearchDir(dir: string, roots: readonly string[]): Promise<string> {
  if (roots.length === 0) {
